...
async function performSearch(query: string, count = 10, offset = 0, type = "web") {
    ...
    return results.map((r: SearchResult) => `Title: ${r.title}\nDescription: ${r.description}\nURL: ${r.url}`).join('\n\n');
}
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
        ...
        if (name === "brave_web_search") {
            const { query, count = 10, offset = 0, type = "web" } = args as { 
                query: string; 
                count?: number;
                offset?: number;
                type?: "web" | "news" | "videos";
            };
